...
static uint32_t shmFormatToDRM(uint32_t shm) {
    switch (shm) {
...
static uint32_t drmFormatToSHM(uint32_t drm) {
    switch (drm) {
...
CWLSHMBuffer::CWLSHMBuffer(SP<CWLSHMPoolResource> pool_, uint32_t id, int32_t offset_, const Vector2D& size_, int32_t stride_, uint32_t fmt_) {
    ...
    Debug::log(LOG, "SHMBuffer: Creating buffer {:x}", (uintptr_t)this);
    ...
    fmt    = fmt_;
    offset = offset_;
    opaque = pixel_format_is_opaque(shmFormatToDRM(fmt_));

    texture = makeShared<CTexture>(shmFormatToDRM(fmt), (uint8_t*)pool->data + offset, stride, size_);
...
CWLSHMBuffer::~CWLSHMBuffer() {
    Debug::log(LOG, "SHMBuffer: Destroying buffer {:x}", (uintptr_t)this);
...
SSHMAttrs CWLSHMBuffer::shm() {
    ...
    attrs.success = true;
    attrs.fd      = pool->fd;
    attrs.format  = shmFormatToDRM(fmt);
    attrs.size    = size;
    attrs.stride  = stride;
    attrs.offset  = offset;
    return attrs;
...
void CWLSHMBuffer::update(const CRegion& damage) {
    texture->update(shmFormatToDRM(fmt), (uint8_t*)pool->data + offset, stride, damage);
}
...
void CWLSHMProtocol::bindManager(wl_client* client, void* data, uint32_t ver, uint32_t id) {
    if (shmFormats.empty()) {
        ...
        for (size_t i = 0; i < len; ++i) {
            shmFormats.push_back(drmFormatToSHM(formats[i]));
        }
    }
