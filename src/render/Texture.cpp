...
#include "../protocols/ToplevelExportWlrFuncs.hpp"
...
CTexture::CTexture(uint32_t drmFormat, uint8_t* pixels, uint32_t stride, const Vector2D& size_) {
    ...
    const wlr_pixel_format_info* format = drm_get_pixel_format_info(drmFormat);
    ASSERT(format);
    auto glFmt = g_pHyprOpenGL->getPixelFormatFromDRM(drmFormat);

    m_iType = pixel_format_is_opaque(drmFormat) ? TEXTURE_RGBX : TEXTURE_RGBA;
...
#endif
    GLCALL(glPixelStorei(GL_UNPACK_ROW_LENGTH_EXT, stride / format->bytes_per_block));
    GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, glFmt->glInternalFormat ? glFmt->glInternalFormat : glFmt->glFormat, size_.x, size_.y, 0, glFmt->glFormat, glFmt->glType, pixels));
...
CTexture::CTexture(const SDMABUFAttrs& attrs, void* image) {
    ...
    m_vSize     = attrs.size;
    m_iType     = pixel_format_is_opaque(attrs.format) ? TEXTURE_RGBX : TEXTURE_RGBA;
    m_pEglImage = image;
    ...
    GLCALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
    GLCALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
    g_pHyprOpenGL->m_sProc.glEGLImageTargetTexture2DOES(m_iTarget, image);
    GLCALL(glBindTexture(GL_TEXTURE_2D, 0));
}
...
void CTexture::update(uint32_t drmFormat, uint8_t* pixels, uint32_t stride, const CRegion& damage) {
    ...
    for (auto& rect : rects) {
        ...
        GLCALL(glTexSubImage2D(GL_TEXTURE_2D, 0, rect.x1, rect.y1, width, height, glFmt->glFormat, glFmt->glType, pixels));
    }
